...
class Compound3DKit(object):
    ...
    def get_MMFF_atom_poses(mol, numConfs=None, return_energy=False):
        ...
        atom_poses = torch.tensor(conf.GetPositions(), dtype=torch.float32)
        if return_energy:
...
def mol_to_graph_data(mol:Mol, smiles=None, atom_poses=None):
    """
    mol_to_graph_data
    ...
    data['edges'] = torch.tensor([row, col], dtype=torch.long)
    for name in bond_id_names:
        data[name] = torch.tensor(data[name])

    perm = (data['edges'][0] * N + data['edges'][1]).argsort()
    data['edges'] = data['edges'][:, perm]
    for name in bond_id_names:
        data[name] = data[name][perm]

    ### make ndarray and check length
    for name in atom_id_names:
        data[name] = torch.tensor(data[name], dtype=torch.long)
    data['mass'] = torch.tensor(data['mass'], dtype=torch.float32)

    pos = atom_poses
    if smiles is None:
        smiles = Chem.MolToSmiles(mol)

    graph_data = Data(atom_type=data['atomic_num'], pos=pos, edge_index=data['edges'], edge_type=data['bond_type'],
                rdmol=copy.deepcopy(mol), smiles=smiles)

    ### morgan fingerprint
    atom_id_names.remove("atomic_num")
    bond_id_names.remove("bond_type")
    for name in atom_id_names:
        graph_data[name] = data[name]
    graph_data['mass'] = data['mass']
    for name in bond_id_names:
        graph_data[name] = data[name]
    graph_data['morgan_fp'] = torch.tensor(CompoundKit.get_morgan_fingerprint(mol), dtype=torch.long)
    # data['morgan2048_fp'] = np.array(CompoundKit.get_morgan2048_fingerprint(mol), 'int64')
    graph_data['maccs_fp'] = torch.tensor(CompoundKit.get_maccs_fingerprint(mol), dtype=torch.long)
    graph_data['daylight_fg_counts'] = torch.tensor(CompoundKit.get_daylight_functional_group_counts(mol), dtype=torch.long)
    return graph_data


def mol_to_geograph_data(mol, atom_poses, smiles=None):
    """
    mol: rdkit molecule
    ...
    if len(mol.GetAtoms()) == 0:
        return None
    data = mol_to_graph_data(mol, smiles=smiles, atom_poses=atom_poses)
    return data


def mol_to_geograph_data_MMFF3d(mol, smiles=None):
    """tbd"""
    if len(mol.GetAtoms()) <= 400:
        mol, atom_poses = Compound3DKit.get_MMFF_atom_poses(mol, numConfs=10)
    else:
        atom_poses = Compound3DKit.get_2d_atom_poses(mol)
    return mol_to_geograph_data(mol, atom_poses, smiles=smiles)
...
if __name__ == "__main__":
    ...
    print(len(smiles))
    print(mol)
    data = mol_to_geograph_data_MMFF3d(mol)
