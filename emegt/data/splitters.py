...
def scaffold_split(dataset, task_idx=None, null_value=0,
    ...
    for i in range(len(dataset)):
        smiles_list.append(dataset[i].smiles)
...
def balanced_scaffold_split(dataset, task_idx=None, null_value=0,
    ...
    n_total_train = len(dataset) - n_total_valid - n_total_test
    ...
    for index_set in index_sets:
        if len(train_idx) + len(index_set) <= n_total_train:
            train_idx += index_set
        elif len(valid_idx) + len(index_set) <= n_total_valid:
            valid_idx += index_set
        else:
            test_idx += index_set
